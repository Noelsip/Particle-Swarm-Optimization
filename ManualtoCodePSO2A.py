...
for index in range(n):
    ...
    vix["v1"] = vix_func(vix["v1"],xi["x0"],0)
    vix["v2"] = vix_func(vix["v2"],xi["x1"],1)
    vix["v3"] = vix_func(vix["v3"],xi["x2"],2)
    
    #update nilai viy berdasarkan fungsi viy_func
    viy["v1"] = viy_func(viy["v1"],yi["y0"],0)
    viy["v2"] = viy_func(viy["v2"],yi["y1"],1)
    viy["v3"] = viy_func(viy["v3"],yi["y2"],2)
    ...
    xi["x0"] = xi_before["x0"] + vix["v1"]
    xi["x1"] = xi_before["x1"] + vix["v2"]
    xi["x2"] = xi_before["x2"] + vix["v3"]
    ...
    yi["y0"] = yi_before["y0"] + viy["v1"]
    yi["y1"] = yi_before["y1"] + viy["v2"]
    yi["y2"] = yi_before["y2"] + viy["v3"]
    
    #Menampilkan nilai xi iterasi sekarang
    for i in range(0,3):
        print(f"Nilai (x{i},y{i}): {xi[f'x{i}']},"+f"{yi[f'y{i}']}")
        print(f"Nilai f(x{i},y{i}): {func(xi[f'x{i}'],yi[f'y{i}'])}")
        print()
